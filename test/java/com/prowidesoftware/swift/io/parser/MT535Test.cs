...
//JAVA TO C# CONVERTER TODO TASK: This Java 'import static' statement cannot be converted to .NET:
	...
	/// @since 4.0
	/// </summary>
	public class MT535Test : BaseMessageTestcase
	{
...
//ORIGINAL LINE: @Test public void test535_1()
		public virtual void test535_1()
		{
			...
			assertEquals("535", (parseMessage(messageToParse)).Type);

			//check b1
			assertEquals("F01FOOBARXXAXXX3219604112", b1.BlockValue);
			assertEquals("F", b1.ApplicationId);
			assertEquals("01", b1.ServiceId);
			assertEquals("FOOBARXXAXXX", b1.LogicalTerminal);
			assertEquals("3219", b1.SessionNumber);
			assertEquals("604112", b1.SequenceNumber);

			//check b2
			assertEquals("I535FOOBARXXXXXXN", b2.BlockValue);
			assertEquals("535", ((SwiftBlock2Input)b2).MessageType);
			...
			assertEquals("GENL", tags[0].Value);
			assertEquals("ADDINFO", tags[1].Value);
			...
			assertEquals(":SFRE//DAIL", tags[0].Value);
			assertEquals(":CODE//COMP", tags[1].Value);
			assertEquals(":STTY//CUST", tags[2].Value);
			assertEquals(":STBA//TRAD", tags[3].Value);

			assertEquals(":SAFE//ABC", b4.getTagValue("97A"));

			tags = b4.getTagsByName("17B");
			assertEquals(":ACTI//Y", tags[0].Value);
			assertEquals(":CONS//Y", tags[1].Value);
			...
			assertEquals(":HOLP//USD0,", tags[0].Value);
			assertEquals(":HOLS//USD0,", tags[1].Value);
